...
A coupled multi-physics MUSCLE3 workflow combining free-boundary equilibrium reconstruction, core transport, and beam-tracing electron cyclotron heating.
...
- **Method A: Set `TORBEAM_EXE` in `env.sh` (Recommended)**
  Edit `workflows/nice_torax_torbeam/env.sh` (or `workflows/nice_torax_torbeam_modern/env.sh`):
  ```bash
...
Use the standard PDS case materialization script:

```bash
export SCENARIOS_REPO=/work/projects/pds/pds-scenarios   # default location
bin/pds-create-case nice_torax_torbeam 105099           # -> cases/nice_torax_torbeam_105099
```

For the modern case structure:
```bash
...
- `cases/overrides/nice_torax_torbeam_<shot>.ymmsl` (shot-specific overrides)
...
### 2. Submit to Slurm
Submit the materialized case to the cluster queue:

```bash
sbatch bin/pds-run-case.sbatch cases/nice_torax_torbeam_105099
```
...
```bash
module load PDS
python3 workflows/nice_torax_torbeam/compare_to_baseline.py cases/runs/nice_torax_torbeam_105099_<timestamp>
```
